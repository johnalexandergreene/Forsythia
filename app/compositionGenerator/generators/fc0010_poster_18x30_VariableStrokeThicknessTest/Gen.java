...
public class Gen extends FCRIG_Basic{
  ...
  public static final float MAXSTROKETHICKNESS=0.02f;
  ...
  public BufferedImage getImage(){
    ...
    //stroke composition polygons
    System.out.println("stroking");
    double 
      maxdepth=getMaxDepth(composition)*1.01,
      depth;
    System.out.println("maxdepth="+maxdepth);
    float strokethickness;
    g.setPaint(STROKECOLOR);
    for(FPolygon p:composition.getPolygons()){
      depth=p.getPolygonDepth();
      strokethickness=(float)(((maxdepth-depth)/maxdepth)*MAXSTROKETHICKNESS);
//      strokethickness=(float)(((maxdepth-depth)/maxdepth));
      System.out.println("relativedepth="+((maxdepth-depth)/maxdepth)+"depth="+depth+" strokethickness="+strokethickness);
      g.setStroke(
  ...
  private int getMaxDepth(ForsythiaComposition composition){
    int maxdepth=-1,d;
    for(FPolygon p:composition.getLeafPolygons()){
      d=p.getPolygonDepth();
      if(d>maxdepth)
